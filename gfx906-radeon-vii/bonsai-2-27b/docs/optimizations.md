...
ngram や 0.8B 下書きモデルによる投機は新規コードで逆効果だった。
高速化ビルド (opt11 + KV f16) でも効き、下書き 2 で新規コード 53.1 → 63.4 t/s (+19%)、書き換え 51.7 → 72.9 t/s (+41%)。
本体が速くなった分だけ検証の割合が重くなり、最適な下書き長は 3 から 2 に縮んだ (新規コードでは 3 以上で逆に遅くなる)。
