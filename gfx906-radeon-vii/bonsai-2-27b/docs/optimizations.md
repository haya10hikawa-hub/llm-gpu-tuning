...
| opt13 | norm+FWHT の読み込みを 8 本同時に、`-ub 1024` | 約 50 | 273 |
| opt13 (332ee24) | 畳み込み入力 CONCAT のタイル化、細い m の split_k | 同上 | 284 |
| **opt14 (bd9f8cf)** | mat-vec 32 重み/スレッド、GQA 6 用 FA タイル、小さい m の bf16 に大きい WG | **約 54 (32k で +22%)** | **284** |
...
**5. 律速は測って確かめる。**
mat-vec から重みの読み込みを抜くと 24.6 µs、活性化を抜くと 37.4 µs (元 40.5 µs) で、命令とメモリ待ちが
半々だった。1 スレッドの担当を 32 重みにして命令を減らすと 1 割速くなった。FA は GQA 6 に対して 8 行タイルを
4 分割していて、1 サブグループが空行を担当していた (32k 文脈で FA −23%)。
