...
8. **「部分的に動いているように見える失敗」の検出条件を明示する** (15章参照)
9. **1 件の差を結論にしない。カテゴリを広げて再現を確認する** (16章参照)
...
| 下流タスク性能 | **完了 -> 16章**。HellaSwag 83.25%、エージェンティック 47/48 |
...
perplexity は代理指標にすぎないため、実タスクで確認した。

### HellaSwag (400 タスク)

| モデル | acc_norm | 95% CI |
|---|---|---|
| UD-Q4_K_S (15.36 GB) | **83.25%** | [79.28%, 86.59%] |
| UD-Q2_K_XL (9.83 GB) | 81.00% | [76.87%, 84.54%] |

差 2.25 ポイント。**信頼区間が大きく重なるため有意ではない。**

予測「正答率の劣化は PPL の劣化率より相対的に大きい」は**外れ**:

```
PPL        5.9350 -> 6.2641   +5.55% (相対)
HellaSwag  83.25% -> 81.00%   -2.7%  (相対)
```

実タスクの劣化の方が小さかった。「低ビット量子化は PPL が示す以上に推論能力を
削る」という経験則は、このモデル・このタスクでは成立しない。

### エージェンティック評価 (自作ハーネス、計 48 タスク)
...
**48 タスクを通じて検出可能な差はない。** 9.83 GB まで削ってもエージェント能力は保たれる。

予測「エージェンティックなタスクは量子化の損傷が顕著に出る」は**外れ**。
...
生成トークン     136 ~ 197
thinking トークン  0   <- tool calling 時に <think> ブロックを出さない
10 ステップのタスク  約 70 秒
```
...
**会議時刻が指定されないと 10:00 を捏造する** (両量子化で同一)。

```
U07  "Schedule a retro called Sprint Retro on 2026-12-01."  -> start_time=10:00
```

他の 11 種類の欠落 (宛先 / 送金先 / 送金額 / 出発地 / 到着地 / 元通貨 / 変換先 /
金額 / 日付 / 本文 / リマインダ時刻) では正しく聞き返す。
「時刻には既定値がある」という学習された前提と思われる。
**エージェント運用では、時刻の既定値を明示的に禁止するシステムプロンプトが要る。**

### 撤回した主張

ラウンド 2 で「Q2_K_XL は引数を捏造する、しかも 35% 遅い」と報告したが、
ラウンド 3 で**どちらも再現しなかった** (同点、所要時間は 8% 速い)。

n=1 の差を発見として報告したのが誤り。規約 #6 (効果量を外挿しない) と
同じ過ちを統計の側で繰り返した。

-> 規約 #9: **1 件の差を結論にしない。カテゴリを広げて再現するか確認する。**

### 総合

| 用途 | 推奨 |
|---|---|
| 通常利用 / エージェント | `UD-Q4_K_S` (decode 26.26, 最良品質、VRAM に収まる) |
| VRAM 12 GB 級 | `UD-Q2_K_XL` でもエージェント能力は同等。PPL と HellaSwag のみ劣る |
| 長文プロンプト中心 | `Q3_K_L` (prefill 208.7) |
